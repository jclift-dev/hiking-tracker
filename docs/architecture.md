...
`country` and `admin1` (ISO codes) are added by `enrich_regions.py` for all European lands (`EU_LANDS` set — 22 values). Swiss stages also carry `cantons` (array of Swiss canton codes), `arrival_stations` (SBB station names for transport lookups), and `sbb_times` (cached travel times from scraper.py). `duration_hrs` comes from the Waymarked Trails API metadata and is present on OSM-scraped non-Swiss routes. Website-only routes (scraper_websites.py etc.) may omit `duration_hrs` if the source doesn't publish it.
