...
10. **`scraper_osm.py`** — Long-distance trails via Waymarked Trails API (19+ countries). Data © OpenStreetMap contributors, ODbL 1.0. CLI: `--only <osm_id>`, `--refresh-trail <osm_id>`, `--skip-elevation`, `--backfill-elevation`, `--backfill-names`, `--backfill-ch-osm-ids` (assigns `_osm_id` to ch-hike routes 1–7 from OSM parent superroutes), `--backfill-swcp-osm-ids` (assigns `_osm_id` to SWCP stages from OSM super-relation 2376086 by position), `--backfill-whw-osm-ids` (assigns `_osm_id` to WHW stages from OSM super-relation 16287 by position).
